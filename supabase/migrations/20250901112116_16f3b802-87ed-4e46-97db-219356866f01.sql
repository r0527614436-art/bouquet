-- Add subcategory field to catalog_items table
ALTER TABLE public.catalog_items 
ADD COLUMN subcategory text DEFAULT NULL;

-- Add an index for better performance on subcategory queries
CREATE INDEX idx_catalog_items_subcategory ON public.catalog_items(subcategory);

-- Create function to generate auto-numbered titles for new items
...
-- Create trigger for auto-numbering (only for new items)
CREATE TRIGGER auto_number_items
    BEFORE INSERT ON catalog_items
    FOR EACH ROW
    EXECUTE FUNCTION generate_item_title();

-- Create function to reorder item numbers when an item is deleted
