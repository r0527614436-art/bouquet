...
const HomepagePopupManager: React.FC = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['homepage-popup-admin'] });
      queryClient.invalidateQueries({ queryKey: ['homepage-popup'] });
  ...
  if (!form) {
    return (
      <Card className="mb-8">
        <CardHeader><CardTitle>פופאפ דף הבית</CardTitle></CardHeader>
        <CardContent>טוען...</CardContent>
      </Card>
    );
  }
